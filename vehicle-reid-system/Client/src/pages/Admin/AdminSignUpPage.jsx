...
export default function AdminSignUpPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate directly to the admin sign-in page upon successful account creation
      navigate("/admin/signin", { replace: true });
    } catch (err) {
      setFormError(err.message || "Admin signup failed. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center px-6 py-12 sm:px-10">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm mb-8" style={{ color: "#4B617D" }}>
            ...
            <Link to="/admin/signin" className="font-semibold underline underline-offset-2" style={{ color: SAPPHIRE }}>
              Sign in
            </Link>
          </p>
          ...
          <p className="mt-8 text-center text-[11px]" style={{ color: "#93A2B8" }}>
            Creating an account provisions full administrative privileges for system approvals.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative flex-1 flex flex-col items-center justify-center gap-5">
          ...
          <p className="text-sm max-w-xs text-center" style={{ color: "#B7CBE2" }}>
            Admins review and approve every camera before it feeds the
            tracking pipeline.
          </p>
        </div>
